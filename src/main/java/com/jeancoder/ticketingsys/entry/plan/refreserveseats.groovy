...
try {
	...
	DataTcSsReserveOrderInfo order = OrderService.INSTANCE.getReserveOrderByNo(order_no);
	DataTcSsReserveOrderRemote remote = OrderService.INSTANCE.getReserveRemoteByOrderNo(order_no);
	List<DataTcSsReserveOrderSeat> seats = OrderService.INSTANCE.getReserveSeatsByOrderNo(order_no);
	...
	for(String sid : seat_ids.split(",")) {
		int leftSeatNum = OrderService.INSTANCE.refReserveSeat(order_no, sid);
		if(leftSeatNum == 0) {
