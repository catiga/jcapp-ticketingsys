...
try {
	List<CinemaAuthInfo> cinemaAuthInfos = StoreService.INSTANCE.getCinemaAuthInfo();
	if(cinemaAuthInfos == null) {
		result.setData(Res.Failed(Codes.COMMON_CINEMA_CONFIG_ERROR));
		return result;
	}
	Iterator<CinemaAuthInfo> it = cinemaAuthInfos.iterator();
	while(it.hasNext()) {
		CinemaAuthInfo cai = it.next();
		if(cai.pid.toString()!=pid.toString()) {
			it.remove();
		}
	}
	logger.info('pid=' + pid + ', ' + cinemaAuthInfos);
	if(cinemaAuthInfos == null) {
		result.setData(Res.Failed(Codes.COMMON_CINEMA_CONFIG_ERROR));
		return result;
	}
	...
	List<CinemaPlanResult> lists = new ArrayList();
	for (cinemaAuthInfo in cinemaAuthInfos) {
		logger.info('cinema_auth:' + JackSonBeanMapper.toJson(cinemaAuthInfo));
